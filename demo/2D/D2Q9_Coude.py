...
class Canvas(app.Canvas):
    ...
    def __init__(self, dico):
        ...
        deltarho = rhoo
        self.min, self.max = 0., uo**2
        self.ccc = 1./(self.max-self.min)
        self.size = W * coeff, H * coeff
        self.program = gloo.Program(VERT_SHADER, FRAG_SHADER)
        self.texture = gloo.Texture2D(self.sol.m[0][0,:].astype(np.float32))
    ...
    def maj(self):
        ...
        #dummy = 0.5+0.5*self.ccc*(self.sol.m[0][1,:,:]**2+self.sol.m[0][2,:,:]**2)
        #dummy[self.indout[1], self.indout[0]] = 0.
        #self.texture.set_data(dummy.astype(np.float32))
        self.sol.m[0][:,self.indout[1], self.indout[0]] = 1.
...
def initialization_qx(x,y):
    return rhoo*uo * np.zeros((x.shape[0], y.shape[0]), dtype='float64')
...
def bc_in(f, m, x, y, scheme):
    m[0][0] = rhoo
    m[0][1] = rhoo*uo * (ymax-y)*(y-0.75*(ymax-ymin))*8**2
    m[0][2] = 0.
...
if __name__ == "__main__":
    ...
    dico = {
        'box':{'x':[xmin, xmax], 'y':[ymin, ymax], 'label':[1, 0, 0, 2]},
        'elements':{0: {'element':Parallelogram((xmin,ymin),(0.75*(xmax-xmin),0),(0,0.75*(ymax-ymin))), 'label':0, 'del':0}},
        'space_step':dx,
        'number_of_schemes':1,
        'scheme_velocity':la,
        ...
        'boundary_conditions':{
            ...
            1:{'method':{0: pyLBMBound.neumann_horizontal}, 'value':None},
