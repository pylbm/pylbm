...
def init_E(x, y, gamma):
    Ec = (
        0.5
        * (init_qx(x, y, gamma) ** 2 + init_qy(x, y, gamma) ** 2)
        / init_rho(x, y, gamma)
    )
...
def run(dx, Tf, generator="cython", sorder=None, with_plot=True):
    ...
    """
    # parameters
    xmin, xmax, ymin, ymax = 0.0, 2 * np.pi, 0.0, 2 * np.pi
    gamma = 5.0 / 3.0
    ...
    dico = {
        ...
        "schemes": [
            ...
            },
            {
                ...
                "equilibrium": [Bx, 0, (qy * Bx - qx * By) / rho, 0.0],
            },
            {
                "velocities": list(range(1, 5)),
                ...
                "polynomials": [1, LA * X, LA * Y, X**2 - Y**2],
                "relaxation_parameters": s_B,
                "equilibrium": [By, (qx * By - qy * Bx) / rho, 0, 0.0],
            },
        ],
        "init": {
            rho: (init_rho, (gamma,)),
