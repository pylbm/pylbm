...
def run(space_step, final_time, generator="cython", sorder=None, with_plot=True):
    ...
    # parameters
    scheme_name = "Geier"
    xmin, xmax, ymin, ymax = 0.0, 2.0, 0.0, 1.0  # bounds of the domain
    radius = 1.0 / 32  # radius of the obstacle
    la = 1.0  # velocity of the scheme
    rho_o = 1.0  # reference value of the mass
    u_o = 0.05  # boundary value of the velocity
    mu = 5.0e-6  # bulk viscosity
    ...
    def moments_choice(scheme_name, mu, zeta):
        ...
        if scheme_name == "Lallemand":
            ...
            equilibrium = [
                ...
                4 / 3 * QX * LA**2,
                4 / 3 * QY * LA**2,
                ((21 * Q2 + 6 * RHO * LA**2) * LA**2 - (6 * Q2 - 2 * RHO * LA**2))
                / 9,
                QX2 - QY2,
                QXY,
            ]
        ...
        s = [0.0, 0.0, 0.0, s_1, s_1, s_1, s_1, s_2, s_2]
    ...
    simu_cfg = {
        ...
        "elements": [
            pylbm.Circle([0.3, 0.5 * (ymin + ymax) + 2 * space_step], radius, label=2)
        ],
        ...
        "parameters": {LA: la},
        "init": {RHO: rho_o, QX: rho_o * u_o, QY: 0.0},
        "boundary_conditions": {
            0: {
    ...
    if with_plot:
        ...
        axe.ellipse(
            [0.3 / space_step - 1.5, 0.5 * (ymin + ymax) / space_step + 0.5],
            [radius / space_step, radius / space_step],
            "black",
        )
        surf = axe.SurfaceImage(vorticity(sol), cmap="jet", clim=[0, 0.1])
