...
def qx_init(x, y, U, k):
    ...
    return (
        np.zeros_like(x)
        + U * np.tanh(k * (y - 0.25)) * (y <= 0.5)
        + U * np.tanh(k * (0.75 - y)) * (y > 0.5)
    )
...
def qy_init(x, y, U, delta):
    ...
    """
    return np.zeros_like(y) + U * delta * np.sin(2 * np.pi * (x + 0.25))
...
def run(space_step, final_time, generator="cython", sorder=None, with_plot=True):
    ...
    zeta = 0.0366  # bulk viscosity
    mu = 1.0e-6  # shear viscosity
    k = 80
    delta = 0.05
    ...
    simu_cfg = {
        ...
        "init": {RHO: rho_o, QX: (qx_init, (u_o, k)), QY: (qy_init, (u_o, delta))},
    ...
    if with_plot:
        ...
        surf = axe.SurfaceImage(vorticity(sol), cmap="jet")
