...
class ShallowWaterSolver(GenericSolver):
    ...
    def _compute_interstate(self):
        ...
        q_star = 0.5 * (self._f1(h_star)[0] + self._f2(h_star)[0])
    ...
    def _f1(self, h_star):
        ...
        if h_star < h_left:  # 1-rarefaction
            q_star = h_star * (u_left + 2 * (c_left - np.sqrt(self.gravity * h_star)))
            dq_star = u_left + 2 * c_left - 3 * np.sqrt(self.gravity * h_star)
        elif h_star > h_left:  # 1-shock
            z = h_star / h_left
            q_star = h_star * (
                u_left - c_left / np.sqrt(2) * (z - 1) * np.sqrt(1 + 1 / z)
            )
            dq_star = (
                u_left
                - c_left / np.sqrt(2) * (2 * z - 1) * np.sqrt(1 + 1 / z)
                + 0.5 * c_left / np.sqrt(2) * (1 - 1 / z) / np.sqrt(1 + 1 / z)
            )
        else:
            q_star = q_left
    ...
    def _f2(self, h_star):
        ...
        if h_star < h_right:  # 2-rarefaction
            q_star = h_star * (u_right - 2 * (c_right - np.sqrt(self.gravity * h_star)))
            ...
            dq_star = (
                u_right
                + c_right / np.sqrt(2) * (2 * z - 1) * np.sqrt(1 + 1 / z)
                + 0.5 * c_right / np.sqrt(2) * (1 - 1 / z) / np.sqrt(1 + 1 / z)
            )
        else:
            q_star = q_right
