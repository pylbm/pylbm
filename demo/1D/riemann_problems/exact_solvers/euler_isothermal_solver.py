...
class EulerIsothermalSolver(GenericSolver):
    ...
    def _compute_interstate(self):
        ...
        """
        rho_star = solve(
            self._f1, self._f2, self.u_left[0], self.u_right[0], self.epsilon
        )
        u_star = 0.5 * (self._f1(rho_star)[0] + self._f2(rho_star)[0])
    ...
    def _compute_waves(self):
        ...
        if self.u_left[0] > self.u_star[0]:  # 1-rarefaction
            self.velocities.append(
                [self.u_left[1] - self.sound_speed, self.u_star[1] - self.sound_speed]
            )
        ...
        if self.u_right[0] > self.u_star[0]:  # 2-rarefaction
            self.velocities.append(
                [self.u_star[1] + self.sound_speed, self.u_right[1] + self.sound_speed]
            )
    ...
    def _f1(self, rho_star):
        ...
        if rho_star < rho_left:  # 1-rarefaction
            ...
            irapport = 1 / rapport
            u_star = u_left - self.sound_speed * (rapport - irapport)
            du_star = -0.5 * self.sound_speed / rho_star * (rapport + irapport)
        else:
            u_star = u_left
            du_star = -self.sound_speed / rho_star
    ...
    def _f2(self, rho_star):
        ...
        if rho_star < rho_right:  # 2-rarefaction
            ...
            irapport = 1 / rapport
            u_star = u_right + self.sound_speed * (rapport - irapport)
            du_star = 0.5 * self.sound_speed / rho_star * (rapport + irapport)
        else:
            u_star = u_right
            du_star = self.sound_speed / rho_star
    ...
    def diagram(self):
        ...
        axes.scatter(
            self.u_left[1], self.u_left[0], color="orange", s=20, label="left state"
        )
        axes.scatter(
            self.u_right[1], self.u_right[0], color="navy", s=20, label="right state"
        )
        axes.scatter(
            self.u_star[1],
            self.u_star[0],
