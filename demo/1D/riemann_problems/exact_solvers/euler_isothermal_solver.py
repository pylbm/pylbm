...
class EulerIsothermalSolver(GenericSolver):
    """
        d_t(rho)   + d_x(rho u) = 0
        d_t(rho u) + d_x(rho u^2 + k rho u) = 0
    
    where k>0

    Assuming that p = k rho, we have
    E = rho u^2/2 + k/(gamma-1) rho
    e = k/(gamma-1) => k = (gamma-1) e
    """
    def _read_particular_parameters(self, parameters):
        self.kappa = parameters.get('kappa', 1.2)
