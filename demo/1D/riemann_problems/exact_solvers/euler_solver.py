...
class EulerSolver(GenericSolver):
    ...
    def _compute_interstate(self):
        ...
        """
        p_star = solve(
            self._f1, self._f2, self.u_left[2], self.u_right[2], self.epsilon
        )
        u_star = 0.5 * (self._f1(p_star)[0] + self._f2(p_star)[0])
        # compute rho_star
        if p_star < self.u_left[2]:  # 1-rarefaction
            rho_star1 = self.u_left[0] * (p_star / self.u_left[2]) ** (1 / self.gamma)
        else:  # 1-shock
        ...
        if p_star < self.u_right[2]:  # 3-rarefaction
            rho_star2 = self.u_right[0] * (p_star / self.u_right[2]) ** (1 / self.gamma)
        else:  # 3-shock
    ...
    def _compute_waves(self):
        ...
        if p_l > p_s1:  # 1-rarefaction
            self.velocities.append([u_l - c_l, u_s1 - c_s1])
        ...
        if p_s2 < p_r:  # 3-rarefaction
            self.velocities.append([u_s2 + c_s2, u_r + c_r])
    ...
    def _f1(self, p_star):
        ...
        if p_star < p_left:  # 1-rarefaction
            ...
            du_star = -np.sqrt(
                (1 - self.mu2) / rho_left / (p_star + self.mu2 * p_left)
            ) * (1 - 0.5 * (p_star - p_left) / (p_star + self.mu2 * p_left))
        else:
            u_star = u_left
            du_star = (
                -facteur
    ...
    def _f2(self, p_star):
        ...
        if p_star < p_right:  # 3-rarefaction
            ...
            du_star = np.sqrt(
                (1 - self.mu2) / rho_right / (p_star + self.mu2 * p_right)
            ) * (1 - 0.5 * (p_star - p_right) / (p_star + self.mu2 * p_right))
        else:
            u_star = u_right
            du_star = (
                facteur
    ...
    def diagram(self):
        ...
        axes.scatter(
            self.u_left[1], self.u_left[2], color="orange", s=20, label="left state"
        )
        axes.scatter(
            self.u_right[1], self.u_right[2], color="navy", s=20, label="right state"
        )
        axes.scatter(
            self.u_star[0][1],
            self.u_star[0][2],
