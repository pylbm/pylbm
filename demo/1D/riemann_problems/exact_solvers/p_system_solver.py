...
class PSystemSolver(GenericSolver):
    ...
    def _read_particular_parameters(self, parameters):
        self.gamma = parameters.get("gamma", 2.0 / 3.0)
    ...
    def _compute_interstate(self):
        ...
        """
        x = 0.5 * (self.u_left[0] + self.u_right[0])
    ...
    def _f1(self, u1_star):
        ...
        if u1_star > u1_left:  # 1-rarefaction
            ...
            du2_star = (
                -(dpu1 * (u1_star - u1_left) + pu1 - pu1_left)
                / np.sqrt((pu1 - pu1_left) * (u1_star - u1_left))
                / 2
            )
        else:
            u2_star = u2_left
    ...
    def _f2(self, u1_star):
        ...
        if u1_star > u1_right:  # 2-rarefaction
            ...
            du2_star = (
                (dpu1 * (u1_star - u1_right) + pu1 - pu1_right)
                / np.sqrt((pu1 - pu1_right) * (u1_star - u1_right))
                / 2
            )
        else:
            u2_star = u2_right
    ...
    def diagram(self):
        ...
        axes.scatter(
            self.u_left[0], self.u_left[1], color="orange", s=20, label="left state"
        )
        axes.scatter(
            self.u_right[0], self.u_right[1], color="navy", s=20, label="right state"
        )
        axes.scatter(
            self.u_star[0],
            self.u_star[1],
