...
def run(space_step, final_time, generator="cython", sorder=None, with_plot=True):
    ...
    """
    # parameters
    xmin, xmax = -1, 1.0  # bounds of the domain
    gnum = 1  # numerical value of g
    la = 2.0  # velocity of the scheme
    s_h, s_u = 1.5, 1.5  # relaxation parameters

    symb_s_h = 1 / (0.5 + SIGMA_H)  # symbolic relaxation parameters
    symb_s_q = 1 / (0.5 + SIGMA_Q)  # symbolic relaxation parameters

    # initial values
    h_left, h_right, q_left, q_right = 2, 1, 0.1, 0.0
    ...
    simu_cfg = {
        ...
        "init": {
            H: (riemann_pb, (xmid, h_left, h_right)),
