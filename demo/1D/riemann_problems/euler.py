...
SIGMA_RHO, SIGMA_U, SIGMA_P = sp.symbols("sigma_1, sigma_2, sigma_3", constants=True)
...
def run(space_step, final_time, generator="cython", sorder=None, with_plot=True):
    ...
    symb_s_p = 1 / (0.5 + SIGMA_P)  # symbolic relaxation parameter
    ...
    rhoe_left = rho_left * u_left**2 + p_left / (gamma - 1.0)
    rhoe_right = rho_right * u_right**2 + p_right / (gamma - 1.0)
    ...
    if with_plot:
        ...
        rhoe_n = sol.m[E]
        u_n = q_n / rho_n
        p_n = (gamma - 1.0) * (rhoe_n - 0.5 * rho_n * u_n**2)
        e_n = rhoe_n / rho_n - 0.5 * u_n**2
        ...
        rhoe_e = 0.5 * sole[0] * sole[1] ** 2 + sole[2] / (gamma - 1.0)
