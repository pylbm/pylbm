...
def run(space_step, final_time, generator="cython", sorder=None, with_plot=True):
    ...
    if velocity > 0:
        xmid = 0.75 * xmin + 0.25 * xmax
    elif velocity < 0:
        xmid = 0.25 * xmin + 0.75 * xmax
    else:
        xmid = 0.5 * xmin + 0.5 * xmax
    # fixed bounds of the graphics
    ymin = min([u_left, u_right]) - 0.2 * abs(u_left - u_right)
    ymax = max([u_left, u_right]) + 0.2 * abs(u_left - u_right)
    ...
    simu_cfg = {
        ...
        "generator": generator,
        "parameters": {LA: la, C: velocity, SIGMA: 1 / s - 0.5},
        "show_code": False,
    }
