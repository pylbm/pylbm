...
def check_from_file(dom, fname):
    ...
    if not create_ref:
        ...
        np.savez(path, in_or_out=dom.in_or_out, distance=dom.distance, flag=dom.flag)


class TestDomain2D:
    dom2d = {
        "box": {"x": [0, 1], "y": [0, 2], "label": 0},
        "space_step": 0.25,
    ...
    def test_simple_domain_with_labels(self):
        ...
        assert np.all(dom.y_halo == [np.linspace(-0.125, 2.125, 10)])
    ...
    def test_domain_with_rectangle(self):
        ...
        dom2d = copy.deepcopy(self.dom2d)
        dom2d["elements"] = [
            pylbm.Parallelogram([0.23, 0.73], [0.5, 0], [0.0, 0.5], label=10)
        ]
        dom = pylbm.Domain(dom2d)
    ...
    def test_domain_with_fluid_rectangle(self):
        ...
        dom2d = copy.deepcopy(self.dom2d)
        dom2d["elements"] = [
            pylbm.Parallelogram([0.0, 0.0], [1.0, 0], [0.0, 2.0], label=20),
            pylbm.Parallelogram(
                [0.23, 0.73], [0.5, 0], [0.0, 0.5], label=10, isfluid=True
            ),
        ]
        dom = pylbm.Domain(dom2d)
    ...
    def test_domain_with_circle(self):
        fname = "circle.npz"
        dom2d = copy.deepcopy(self.dom2d)
        dom2d["elements"] = [pylbm.Circle([0.5, 1.0], 0.5, label=10)]
        dom = pylbm.Domain(dom2d)
    ...
    def test_domain_with_fluid_circle(self):
        ...
        dom2d["elements"] = [
            pylbm.Parallelogram([0.0, 0.0], [1.0, 0], [0.0, 2.0], label=20),
            pylbm.Circle([0.5, 1.0], 0.5, label=10, isfluid=True),
        ]
        dom = pylbm.Domain(dom2d)
    ...
    def test_domain_with_triangle(self):
        ...
        dom2d = copy.deepcopy(self.dom2d)
        dom2d["elements"] = [
            pylbm.Triangle([0.23, 0.73], [0.5, 0], [0.0, 0.5], label=10)
        ]
        dom = pylbm.Domain(dom2d)
    ...
    def test_domain_with_fluid_triangle(self):
        ...
        dom2d["elements"] = [
            pylbm.Parallelogram([0.0, 0.0], [1.0, 0], [0.0, 2.0], label=20),
            pylbm.Triangle([0.23, 0.73], [0.5, 0], [0.0, 0.5], label=10, isfluid=True),
        ]
        dom = pylbm.Domain(dom2d)
