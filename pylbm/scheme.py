...
class Scheme:
    ...
    def generate(self, backend, sorder, valin):
        ...
        #     dicoST = {'ST':ST,
        #               'symb_t':self.symb_t,
        #               'varx':self.symb_coord[0],
        #               'vary':self.symb_coord[1],
        #               'varz':self.symb_coord[2],
        ...
        if backend.upper() == "NUMPY":
            ################## FIX
            #log.error("NUMPY generator not allowed in this version with relative velocities")
            m = indexed('m', [ns, nx, ny, nz], index=[nv] + iloop, ranges=range(ns), permutation=sorder)
    ...
    @staticmethod
    def set_boundary_conditions(f, m, bc, interface):
        """
        ...
        according to the labels. In the direction parallel to the boundary, N denotes
    ...
    #     return np.eye(nvtot) + np.dot(iM, np.dot(C, M))
    ...
    #     """
    #     Jr = self.compute_amplification_matrix_relaxation()
