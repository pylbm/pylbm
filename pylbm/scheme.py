...
class Scheme:
    """
    Create the class with all the needed informations
    for each elementary scheme.
    ...
    dico : dict
        a dictionary that contains the following `key:value`
            ...
            - lattice_velocity : the ratio space step over time step
              (la = dx / dt)
            - lattice_diffusivity : the ratio space step square over time step
              (mu = dx^2 / dt)
            - schemes : a list of dictionaries, one for each scheme
    check_inverse : bool, optional
        True if M * M^{-1} = Identity has to be verified
        default is True
    need_validation : bool, optional
        True if the dictionary has to be verified
        default is True

    Notes
    -----

    The two parameters `lattice_velocity` and `lattice_diffusivity`
    cannot be set simultaneously. They are used to fix the time step.

    Each dictionary of the list `schemes` should contains
    ...
    - relaxation_parameters: list of the relaxation parameters values
    - source_terms: dictionary do define the source terms (optional)
    ...
    s  : list of list of doubles
      ...
      (example: s[k][l] is the parameter associated
      to the lth moment in the kth scheme)
    M : sympy matrix
    ...
    def __init__(self,
        ...
        self.hyp_scaling, self.par_scaling = False, False
        self.symb_mu, self.mu = self._get_var_symbolic(
            dico, 'lattice_diffusivity'
        )
        self.symb_la, self.la = self._get_var_symbolic(
            dico, 'lattice_velocity'
        )
        self.symb_dx, self.dx = self._get_var_symbolic(
            dico, 'space_step'
        )

        if self.mu is not None:  # parabolic scaling
            self.par_scaling = True
            self.symb_la, self.la = 1, 1
        else:  # hyperbolic scaling
            self.hyp_scaling = True
        ...
        self.s = sp.Matrix(
            [r for s in scheme for r in s['relaxation_parameters']]
        )

        # TODO: add the possibility to have vectorial schemes
        #       when M matrix is defined
        if len(scheme) == 1 and 'M' in scheme[0]:
            ...
            self.M, self.invM, self.Tu, self.Tmu = (
                self._create_moments_matrices()
            )
    ...
    def _get_equilibrium(self, scheme):
        ...
        for i, s in enumerate(scheme):
            ...
            if feq and meq:
                log.error(
                    "Error in the creation of the scheme %d"
                    ": you can have only 'feq' or 'equilibrium'",
                    i
                )
                sys.exit()
            if meq:
            ...
            if feq:
                ...
                meq_tmp = self.M[sli, sli] * feq[0](
                    self.stencil.get_all_velocities(i), *feq[1]
                )
                meq_tmp.simplify()
    ...
    def _check_entry_size(self, schemes, key):
        for i, s in enumerate(schemes):
            ...
            if ls != nv:
                log.error(dedent(
                    f"""\
                    the size of the entry for the key {key} in the scheme {i}
                    has not the same size of the stencil {i}: {ls}, {nv}"""
                ))
    ...
    def __str__(self):
        ...
        for k in range(self.nschemes):
            ...
            header_scheme.append(header_string(f"Scheme {k:d}"))
    ...
    def _create_moments_matrices(self):
        """
        Create the moments matrices M and M^{-1} used to transform
        the repartition functions into the moments

        Three versions of these matrices are computed:

          - a sympy version M and invM for each scheme
          - a global numerical version MnumGlob and invMnumGlob
            for all the schemes
        """
        M_, invM_, Mu_, Tu_ = [], [], [], []
        ...
        try:
            for k in range(self.nschemes):
                ...
                for i in range(nvk):
                    for j in range(nvk):
                        ind_x = self.stencil.nv_ptr[k] + i
                        ind_y = self.stencil.nv_ptr[k] + j
                        index = ind_x, ind_y
                        M[index] = M_[k][i, j]
            ...
            log.error(
                "Unable to convert to float the expression "
    ...
    def _check_inverse_of_Tu(self):
        ...
        if not test:
            log.warning(
                "The property on the translation matrix "
                "is not verified\n "
    ...
    def _check_inverse(self, M, invM, matrix_name):
        ...
        if not test:
            log.warning(
                "Problem %s * inv%s is not identity !!!",
    ...
    def _get_indices_cons_noncons(self):
        """
        return the list of the conserved moments
        and the list of the non conserved moments
    ...
    #         # by default no source term
    #         source_terms.append([None]*self.stencil.nv[ns])
    ...
    #     key: the moment concerned should be the name of the moment
    #         as a string or a sympy Symbol or an integer
    #     value: the value of the source term should be a float
    #         or a sympy expression
    #                         """
    #                     ))
    #                     sys.exit()
