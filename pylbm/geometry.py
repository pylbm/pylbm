...
class Geometry:
    ...
    def visualize(
        ...
        if self.dim == 1:
            ...
            lpos = np.asarray(
                [
                    [xmin + length, xmin, xmin, xmin + length],
            ...
            rpos = np.asarray(
                [
                    [xmax - length, xmax, xmax, xmax - length],
                    [-height, -height, height, height],
                ]
            ).T
            ...
            for elem in self.list_elem:
                if elem.isfluid:
                    ...
                    color = "white"
                    alpha_ = 1
                elem.visualize(view, color, viewlabel=viewlabel, alpha=alpha_)
            ...
            couleurs = [(0.5 + 0.5 / k, 0.5 / k, 1.0 - 1.0 / k) for k in range(1, 11)]
            ...
            for k in range(3):
                for x0_ in [pmin[k], pmax[k]]:
                    ...
                    if viewlabel:
                        x = 0.25 * np.sum(coord[(2 - k) % 3])
                        y = 0.25 * np.sum(coord[(3 - k) % 3])
                        z = 0.25 * np.sum(coord[(1 - k) % 3])
            ...
            for elem in self.list_elem:
                if elem.isfluid:
                    ...
                    color = [
                        couleurs[elem.label[k]] for k in range(elem.number_of_bounds)
                    ]
                    alpha_ = alpha
