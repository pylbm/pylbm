...
class EquivalentEquation:
    def __init__(self, scheme):
        ...
        for i in range(all_vel.shape[1]):
            vd = LA * sp.diag(all_vel[:, i].tolist(), unpack=True)
        ...
        for dim, lambda_ in enumerate(Lambda):
            A, B = sp.Matrix([lambda_[:nconsm, :nconsm]]), sp.Matrix(
                [lambda_[:nconsm, nconsm:]]
            )
            C, D = sp.Matrix([lambda_[nconsm:, :nconsm]]), sp.Matrix(
                [lambda_[nconsm:, nconsm:]]
            )
        ...
        self.coeff_order2 = [
            [sp.zeros(nconsm) for _ in range(scheme.dim)] for _ in range(scheme.dim)
        ]
            ...
            for i in range(scheme.dim):
                for jc in range(nconsm):
                    for ic in range(nconsm):
                        self.coeff_order2[dim][i][ic, jc] += (
                            sphi1[ic].expand().coeff(sp.Derivative(func[jc], space[i]))
                        )
    ...
    def __str__(self):
        ...
        t, x, y, z, U, Fx, Fy, Fz, Delta = sp.symbols(
            "t, x, y, z, U, Fx, Fy, Fz, Delta_t"
        )
        ...
        for i in range(self.dim):
            for j in range(self.dim):
                order2.append(
                    sp.pretty(
                        sp.Eq(B[i][j], -Delta * self.coeff_order2[i][j], evaluate=False)
                    )
                )
                phys_equation_rhs += sp.Derivative(
                    B[i][j] * sp.Derivative(U, space[j]), space[i]
                )
        return template.render(
            header=header_string("Equivalent Equations"),
            dim=self.dim,
            ...
            order1=[
                sp.pretty(sp.Eq(F, coeff, evaluate=False))
                for F, coeff in zip([Fx, Fy, Fz][: self.dim], self.coeff_order1)
            ],
            order2=order2,
        )
    ...
    def vue(self):
        ...
        for i in range(self.dim):
            for j in range(self.dim):
                order2_dict[sp.latex(B[i][j])] = [
                    sp.latex(-Delta * c) for c in self.coeff_order2[i][j]
                ]
            ...
            children=[
                ...
                v.Row(
                    children=[
                        widgets.HTMLMath(
                            sp.latex(
                                sp.Eq(phys_equation, phys_equation_rhs),
                                mode="equation*",
                            )
                        )
                    ],
                        ...
                        v.ExpansionPanel(
                            children=[
                                v.ExpansionPanelHeader(
                                    children=["Conserved moments"], class_="title"
                                ),
                                v.ExpansionPanelContent(
                                    children=[
                                        v.Row(
                                            children=[
                                                widgets.HTMLMath(
                                                    order0_template.render(consm=consm)
                                                )
                                            ],
                                            justify="center",
                                        )
